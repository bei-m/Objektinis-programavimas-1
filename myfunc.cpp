...
bool galutinis(studentas &temp)
{
    ...
    temp.nd.clear();
    temp.nd.shrink_to_fit();
    if (temp.gal_v>=5||temp.gal_m>=5) {return true;}
    else {return false;}
}
...
void spausdinti(vector<studentas> &grupe)
{
    ...
    {
     cout<<left<<setw(19)<<grupe[i].pavarde<<left<<setw(16)<<grupe[i].vardas;
     cout<<"    "<<setw(14)<<fixed<<setprecision(2)<<grupe[i].gal_v<<"    "<<setw(14)<<fixed<<setprecision(2)<<grupe[i].gal_m<<endl;
    }
...
void pildyti(int stud, int paz, string &pav)
{
    ...
    do
    {
        cout<<"Įrašykite norimą failo pavadinimą: ";
        c=check(f,1);
    } while (!c);
    pav=f;
    std::ofstream failas (f+".txt");
    ...
    try
    {
        ...
        if (ch=="F"||ch=="f") 
        {
            ...
            for (int k=0; k<grupe.size(); k++)
            {
                b=galutinis(grupe[k]);
                if (b)
                {grupe_2.push_back(grupe[k]);}
                else {grupe_1.push_back(grupe[k]);}
            }
            a=grupe_1.size()+grupe_2.size();
            ...
            for (int j=0; j<grupe_2.size(); j++)
            {
                rez1<<setw(20)<<left<<grupe_2[j].pavarde<<setw(16)<<left<<grupe_2[j].vardas;
                rez1<<setw(16)<<fixed<<setprecision(2)<<grupe_2[j].gal_v<<setw(16)<<fixed<<setprecision(2)<<grupe_2[j].gal_m<<endl;
            }
            ...
            for (int j=0; j<grupe_1.size(); j++)
            {
                rez2<<setw(20)<<left<<grupe_1[j].pavarde<<setw(16)<<left<<grupe_1[j].vardas;
                rez2<<setw(16)<<fixed<<setprecision(2)<<grupe_1[j].gal_v<<setw(16)<<fixed<<setprecision(2)<<grupe_1[j].gal_m<<endl;
            }
            grupe_1.clear(); grupe.shrink_to_fit();
            auto e5 = high_resolution_clock::now();
            diff=e1-s1;
            cout<<"\n\n";
            cout<<a<<" įrašų failo nuskaitymo laikas: "<<diff.count()<<" s\n";
            diff=e2-e1;
            cout<<a<<" įrašų failo rūšiavimo abėcėlės tvarka laikas: "<<diff.count()<<" s\n";
            diff=e3-s3;
            cout<<a<<" įrašų failo galutinių pažymių skaičiavimo ir dalijimo į dvi grupes laikas: "<<diff.count()<<" s\n";
            diff=e4-s4;
